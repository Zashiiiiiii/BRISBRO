...
BEGIN
  ...
  -- Check if an approved submission already exists for this household
  ...
  ELSE
    ...
    INSERT INTO ecological_profile_submissions (
      ...
      respondent_name, respondent_relation, interview_date,
      ...
      p_respondent_name, p_respondent_relation, v_interview_date,
...
-- Clean up duplicate submissions for household 2, keeping only the latest
DELETE FROM ecological_profile_submissions
WHERE id NOT IN (
  SELECT DISTINCT ON (household_number) id
  FROM ecological_profile_submissions
  WHERE status = 'approved' AND deleted_at IS NULL
  ORDER BY household_number, reviewed_at DESC NULLS LAST, created_at DESC
)
AND status = 'approved'
AND deleted_at IS NULL;
